...
class Condition {
  public:
    Condition(char* debugName, Lock* lock = NULL);		// initialize condition to 
					// "no one waiting"
    ~Condition();			// deallocate the condition
    char* getName() { return (name); }
    Lock* getLock() { return condLock; }
...
class Barrier {
  public:
    Barrier(char* debugName, int tnum);
    ~Barrier();
    void useBarrier();
    char* getName() { return name; }

  private:
    bool broken; // barrier is in use or broken
    char* name;
    Condition* cond;
    int needed_num;
    int now_num;
