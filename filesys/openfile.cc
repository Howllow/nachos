...
{
   synchDisk->rwlock[hdr->sector_num]->ReadAcquire(); 
   int result = ReadAt(into, numBytes, seekPosition);
   //printf("Thread %s has read\n", currentThread->getName());
   //printf("reader num:%d\n", synchDisk->rwlock[hdr->sector_num]->readers);
   //currentThread->Yield();
   seekPosition += result;
   synchDisk->rwlock[hdr->sector_num]->ReadRelease(); 
   return result;
}
...
{
   synchDisk->rwlock[hdr->sector_num]->WriteAcquire(); 
   int result = WriteAt(into, numBytes, seekPosition);
   //printf("Thread %s has written\n", currentThread->getName());
   //currentThread->Yield();
   seekPosition += result;
   synchDisk->rwlock[hdr->sector_num]->WriteRelease(); 
   return result;
}
...
OpenFile::WriteAt(char *from, int numBytes, int position)
{
    ...
    if ((position + numBytes) > fileLength) {
        ...
        if (!hdr->DynamicAdd(position + numBytes - fileLength, freeMap)) {
            numBytes = fileLength - position;
        }
        hdr->WriteBack(hdr->sector_num);
